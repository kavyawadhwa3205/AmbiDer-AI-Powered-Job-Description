...
function Reports() {
  ...
  const generatePDF = async (html, filename, setGen) => {
    ...
    try {
      ...
      el.style.cssText =
        "font-family: Inter, Arial, sans-serif; padding: 40px; color: #2F2F2F; width: 780px; background: white;";
      el.innerHTML = html;
  ...
  const handleJDReport = async () => {
    ...
    await generatePDF(
      html,
      `${(jd.job_title || "JD").replace(/\s+/g, "_")}_Report.pdf`,
      setGenerating1,
    );
  };
  ...
  const handleAnalyticsReport = async () => {
    ...
    const html = `
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:24px;">
        ${[
          ["Total JDs Generated", s.total_jds],
          ["Most Used Industry", s.most_used_industry],
          ["Total Edits Made", s.total_edits],
          ["JDs This Week", s.jds_this_week],
        ]
          .map(
            ([label, val]) => `
  ...
  if (loading)
    return (
      <div className="page-container">
        <div className="empty-state">Loading reports data...</div>
      </div>
    );
    ...
    <div className="page-container">
      ...
      >
        ...
        <p style={{ color: "var(--text-secondary)", fontSize: "14px" }}>
          Download professional branded PDF reports for your JDs
        </p>
      </div>
      ...
      <div className="reports-grid">
        ...
        <div className="report-card">
          ...
          <p>
            Download a branded AmbiDer PDF report for any generated JD with full
          ...
          <div className="report-action-area">
            ...
            <button
              ...
              onClick={handleJDReport}
              disabled={generating1 || jds.length === 0}
            >
        ...
        <div className="report-card">
          ...
          <p>
            Download a summary of your JD generation activity including stats
            and top job titles.
          </p>
          <div className="report-action-area">
            <input
            ...
            <button
              ...
              disabled={generating2 || !analytics}
            >
        ...
        <div className="report-card">
          ...
          <p>
            Select multiple JDs and download them all in one branded PDF file.
          </p>
          <div className="report-action-area bulk-area">
            <div className="bulk-actions">
              <button
                className="text-btn"
                onClick={() => setSelectedBulk(new Set(jds.map((j) => j.id)))}
              >
                Select All
              </button>
              <button
            ...
            <div className="bulk-list">
              ...
              {jds.map((jd) => (
                <div
                  key={jd.id}
                  className="bulk-list-item"
                  onClick={() => toggleBulk(jd.id)}
                >
                  {selectedBulk.has(jd.id) ? (
                    <CheckSquare size={15} color="#E8510A" />
                  ) : (
            ...
            <button
              ...
              disabled={generating3 || selectedBulk.size === 0}
              style={{ marginTop: 12 }}
            >
              {generating3
                ? "Generating..."
                : `Download ${selectedBulk.size > 0 ? `(${selectedBulk.size}) ` : ""}Selected as PDF`}
            </button>
          </div>
        </div>
      </div>

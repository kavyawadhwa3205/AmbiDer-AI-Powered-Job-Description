...
const ENHANCE_OPTIONS = [
  {
    ...
    icon: Briefcase,
    instruction: "Rewrite this JD in a more formal and professional tone",
  },
  {
    label: "ATS Optimized",
    icon: Shield,
    instruction: "Optimize this JD for ATS systems by adding relevant keywords",
  },
  {
    label: "Shorten Content",
    icon: MinusCircle,
    instruction:
      "Make this JD more concise while preserving essential information",
  },
  {
    label: "Expand Content",
    icon: PlusCircle,
    instruction: "Add more detail to each section of this JD",
  },
  {
    label: "Change Tone",
    icon: Smile,
    instruction: null,
    hasToneDropdown: true,
  },
];
...
function parseJDSections(jdText) {
  ...
  for (const line of lines) {
    ...
    if (headerMatch) {
      ...
      const metaEntry = Object.entries(sectionMeta).find(([key]) =>
...
function OutputPage({
  ...
  }, [formData.job_title, formData.skills, jdText]);
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      pdf.text("AmbiDer Job Description Generator", pdfWidth / 2, y, {
        align: "center",
      });
      y += 8;
  ...
  const handleChatSend = async (instruction) => {
    ...
    try {
      ...
      if (data.error) {
        ...
        setChatMessages([
          ...newMsgs,
          { role: "ai", text: "Done! The JD has been updated." },
        ]);
        if (formData.saved_jd_id) {
          try {
            await fetch(`${API_BASE}/save/edit`, {
      ...
      setChatError(
        "Could not reach server. Make sure Flask is running on port 5001.",
      );
    } finally {
  ...
  return (
    <>
        ...
        <div className="jd-content-area">
          <div className="jd-header-bar">
              ...
              <div className="jd-header-info">
                ...
                <div className="jd-header-meta">
                  ...
                  {formData.location && (
                    <>
                      <span className="meta-item">
                        <MapPin className="meta-icon" /> {formData.location}
                      </span>
                      <span className="meta-dot">•</span>
                    </>
                  )}
            ...
            <div className="jd-header-actions">
              <button
                ...
                id="edit-inputs-btn"
              >
                <Pencil className="btn-icon" /> Edit Inputs
              </button>
              <button
                className="header-btn"
                ...
                id="regenerate-btn"
              >
                <RefreshCw className="btn-icon" /> Regenerate JD
              </button>
              <button className="header-btn primary" id="save-jd-btn">
                <Download className="btn-icon" /> Save JD
              </button>
            </div>
          </div>
          ...
          <div ref={jdRef}>
            {sections.map((sec) => {
              ...
              const isSkill =
                sec.key === "skills" || sec.key === "nice-to-have";
              const isBenefit = sec.key === "benefits";
              if (isSkill) {
                const tags = extractTags(sec.content);
                return (
                  <div className="jd-card" key={sec.key}>
                    <div className="jd-section-header">
                      <div className={`jd-section-icon ${sec.color}`}>
                        <IconComp size={20} />
                      </div>
                      <h3>{sec.title}</h3>
                    </div>
                    <div className="jd-skills-tags">
                      {tags.map((t, i) => (
                        <span className="jd-skill-tag" key={i}>
                          {t}
                        </span>
                      ))}
                    </div>
        ...
        <div className="output-right-panel">
          <div className="panel-card">
            <div className="panel-title">
              <Sparkles className="panel-icon" /> AI Enhance
            </div>
              ...
              {ENHANCE_OPTIONS.map((opt) => {
                ...
                if (opt.hasToneDropdown) {
                  return (
                    <div key={opt.label}>
                      <button
                        className={`enhance-item ${enhanceLoading === opt.label ? "loading" : ""}`}
                        onClick={() => setToneOpen(!toneOpen)}
                        disabled={!!enhanceLoading}
                      >
                        <span className="enhance-left">
                          <OptIcon className="enhance-icon" /> {opt.label}
                        </span>
                        {enhanceLoading === opt.label ? (
                          <span className="enhance-spinner" />
                        ) : (
                          <ChevronDown className="enhance-arrow" />
                        )}
                      </button>
                      {toneOpen && (
                        <div style={{ paddingLeft: 34 }}>
                          {TONE_OPTIONS.map((tone) => (
                            <button
                              key={tone}
                              className="enhance-item"
                              onClick={() =>
                                handleEnhance(
                                  `Change the tone of this JD to ${tone}`,
                                )
                              }
                ...
                return (
                  <button
                    className={`enhance-item ${enhanceLoading === opt.instruction ? "loading" : ""}`}
                    key={opt.label}
                    onClick={() => handleEnhance(opt.instruction)}
                    disabled={!!enhanceLoading}
                  >
                    <span className="enhance-left">
                      <OptIcon className="enhance-icon" /> {opt.label}
                    </span>
                    {enhanceLoading === opt.instruction ? (
                      <span className="enhance-spinner" />
                    ) : (
                      <ChevronRight className="enhance-arrow" />
                    )}
                  </button>
                );
              })}
          ...
          <div className="panel-card">
            <div className="panel-title">
              <BarChart2 className="panel-icon" /> JD Insights
            </div>
            <div className="panel-subtitle" />
            <div className="insight-row">
              <div className="insight-label-row">
                <span className="insight-label">Readability</span>
                <span className="insight-value">{readabilityScore}/100</span>
              </div>
            ...
            <div className="insight-row">
              <div className="insight-label-row">
                <span className="insight-label">ATS Score</span>
                <span className="insight-value">
                  {atsLoading ? "..." : `${atsData?.score ?? 0}/100`}
                </span>
              </div>
              <div
          ...
          <div className="panel-card">
            <div className="panel-title">
              <LayoutGrid className="panel-icon" /> Similar Templates
            </div>
            <div className="template-list">
              {[
                ...
                <button
                  ...
                  key={t}
                  onClick={() => onNavigate("form", { job_title: t })}
                >
                  <span>{t}</span>
                  <ChevronRight className="t-arrow" />
                </button>
              ))}
            </div>
          </div>
      ...
      <div className="bottom-action-bar">
        <button className="bottom-btn" onClick={handleCopy} id="copy-jd-btn">
          <Copy className="btn-icon" /> Copy JD
        </button>
        <button
          ...
          id="download-pdf-btn"
        >
          <FileText className="btn-icon red" /> Download PDF
        </button>
        <button
          className="bottom-btn"
        ...
        <button className="bottom-btn" onClick={handleShare} id="share-jd-btn">
          <Share2 className="btn-icon" /> Share JD
        </button>
        <button
          className="bottom-btn improve"
          onClick={() => setChatOpen(true)}
          id="improve-ai-btn"
        >
          <Sparkles className="btn-icon" /> Improve with AI
        </button>
      </div>
          ...
          <div className="chat-panel-slide">
            <div className="chat-panel-header">
              <div className="chat-panel-header-left">
                ...
                <div>
                  <h3>Edit with AI</h3>
                  <p>Type an instruction to modify the JD</p>
                </div>
              </div>
              <button
            ...
            <div className="chat-messages">
              {chatMessages.length === 0 && !chatError ? (
                <div className="chat-empty">
                  <div className="chat-empty-inner">
                    ...
                    <p>
                      Tell me how to edit the JD.
                      <br />
                      For example:
                      <br />
                      "Make it more formal"
                      <br />
                      "Add a line about remote work"
                      <br />
                      "Shorten the responsibilities"
                    </p>
                  </div>
                </div>
              ) : (
              ...
              {chatLoading && (
                <div className="chat-message ai">
                  ...
                  <div className="msg-bubble">Updating the JD...</div>
                </div>
              )}
              {chatError && (
            ...
            <div className="chat-input-area">
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && !chatLoading) handleChatSend();
                }}

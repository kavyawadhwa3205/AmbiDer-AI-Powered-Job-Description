...
function SavedJDs({ onNavigate, onJDLoad }) {
  ...
  const handleDownloadPDF = async (jd) => {
    try {
      ...
      el.style.cssText =
        "font-family: Inter, sans-serif; padding: 40px; color: #2F2F2F; width: 750px;";
      el.innerHTML = `
      ...
      await html2pdf()
        .set({
          margin: 0.75,
          filename: `${jd.job_title.replace(/\s+/g, "_")}_JD.pdf`,
          html2canvas: { scale: 2 },
          jsPDF: { format: "a4" },
        })
        .from(el)
      ...
      alert(
        "PDF generation failed. Please use the View button and download from the Output Page.",
      );
    }
  };
    ...
    <div className="page-container">
      ...
      {loading ? (
        ...
        <div className="jds-grid">
          {filteredJDs.map((jd) => {
            const dateStr = new Date(jd.created_at).toLocaleDateString(
              "en-US",
            ...
            return (
              <div className="jd-list-card" key={jd.id}>
                <div className="jd-card-info">
                  ...
                  {jd.company_name && (
                    <div className="jd-card-company">{jd.company_name}</div>
                  )}
                  <div className="jd-card-meta">
                    {jd.industry && (
                      <span className="industry-tag">{jd.industry}</span>
                    )}
                    <span
                      style={{
                        display: "flex",
